...
def plotBivariate(cfg, region, ax, source='grdc', variable2='P_Q', use_percentile=False, ax_legend=None, plotClimateMap=False):
    ...
    if region=='north_america' and plotClimateMap:
    ...
    if region=='north_america':
        ...
        ax = ax.inset_axes([0.7,0.15,0.35,0.35]) #x,y, W, H
...
def plotCausalEffectLat(cfg, pcnt=None):
    ...
    for ix, region in enumerate(config.data.regions):
        ...
        print ('region', region, 'no ace', len(TWS_ace))
        TWS_ace = np.array(TWS_ace)
        P_ace = np.array(P_ace)
        ...
        for axis in ['top', 'bottom', 'left', 'right']:
                ...
                ax3.spines[axis].set_linewidth(1.0) 

        ax2.set_xlabel('TWSA', fontdict={'fontsize': 15})
...
def plotGRanD(cfg):
    ...
    #12292023, comment out dam layer
    ...
    #12292023
    #plot_handles = [*handles, lmark, pmark]
    plot_handles = [*handles, lmark]
    ax.legend(
...
def printGRDCInfo(cfg):
    ...
    with open('allstationlist.csv', 'w') as fid:
        fid.write("Station ID,River,Drainage Area,Co-occurring Extremes (TWS-Q | P-Q) \n")
...
def compareGRDC_GloFAS(cfg):
    ...
    09212023, switch GLOFAS to new results generated from grdc_monthly_new.py
    Now, the total number of overlapped stations is 85
    """
    if cfg.data.removeSWE:
        swe='swe'
    else:
    ...
    if not cfg.data.deseason:
        glofas_outputDict2 = pkl.load(open(f'grdcresults/glofas_{cfg.event.event_method}_all_events{swe}_new.pkl', 'rb'))
    else:
...
def plotACEMap(cfg,axes,caxes,pcnt=None):
    ...
    for ix, region in enumerate(config.data.regions):
        ...
        dfMetrics = pd.DataFrame({'grdc_no': np.array(validStations,dtype=np.int64), 'TWSA_ACE':TWS_ace, 'P_ACE': P_ace})
        dfStation = dfStation.merge(dfMetrics, on='grdc_no', how='inner')
      
        cmap = ListedColormap(sns.color_palette(palette="rocket_r", n_colors=6).as_hex())    
        for varname,ax,cax in zip(['TWSA', 'P'], axes,caxes):
...
if __name__ == '__main__':    
    ...
    if itask == 1:
        ...
        if source =='grdc':
            ...
            #09062023, add asia as the top-left plot
            ...
            for region,theax in zip(config.data.regions, allax):
                plotBivariate(cfg=config, region=region, ax=theax, source='grdc', variable2='P_Q', ax_legend=ax_legend, plotClimateMap=True)
